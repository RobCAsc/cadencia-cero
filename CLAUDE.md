...
- **Map view** (`src/game/ride/MapView.ts`, the "Mapa" button by Terminar): the paper map of the Route during the ride, a 15-km window (5 behind, 10 ahead) drawn as the board's hand-drawn road: the refuges in the window with their km, today's stretch in gold with a dotted projection to where the ride should end, the rider at their real km, the horde behind sized by closeness, the ghost, the whole ride segment by segment with the playhead and one line saying what is next, and the advantage and pulse so the ride never leaves sight. Any tap outside the button returns to the road. The rider asked for an aerial or isometric second view (2026-09-21); this is the answer within the anti-goals: the game state is one number on a line, so a map shows everything a 3D view could and costs one drawing layer instead of a second renderer.
